...
import { exportOAuthDiagnostics, finishOAuthLogin, getOAuthRuntimeSummary, listOAuthProviders, prepareOAuthLogin, resetAllOAuthProviders, resetOAuthProvider, updateOAuthProvider } from './oauth.service.js';
...
authRouter.patch('/oauth/providers/:provider', authMiddleware, requireRole(['admin', 'enterprise']), validateBody(oauthUpdateSchema), (req, res) => {
    ...
    Promise.resolve(updateOAuthProvider(parsed.data, req.body.enabled, req.user?.email))
...
authRouter.delete('/oauth/providers/:provider/override', authMiddleware, requireRole(['admin', 'enterprise']), (req, res) => {
    ...
    Promise.resolve(resetOAuthProvider(parsed.data, req.user?.email))
        .then((dados) => res.json({ dados }));
});
