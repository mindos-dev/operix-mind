...
import { addTechnicalContext, improveUserPrompt, translatePromptToEnglish } from './context-builder.js';
import { estimateCost, estimateTokens, getAgentExecutionLogs, logAgentDecision, trimPrompt } from './token-economy.js';
...
export function createDeepClawPlan(input) {
    logAgentDecision({
        etapa: 'deep_claw',
        agente: 'Deep Claw',
...
function classifyIntent(input) {
    ...
    if (normalized.includes('dashboard'))
        return 'dashboard';
    if (normalized.includes('orçamento') || normalized.includes('orcamento'))
        return 'sistema_de_orcamento';
    if (normalized.includes('aws') || normalized.includes('deploy'))
        return 'devops';
