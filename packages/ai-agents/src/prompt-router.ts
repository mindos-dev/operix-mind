...
export function routePrompt(request: AgentRequest): AgentDefinition {
  const input = request.input.toLowerCase();

  if (request.taskType === 'coordenacao') return logRoute(deepClawAgent, 'coordenação geral solicitada');
...
export async function executeRoutedPrompt(request: AgentRequest) {
  const agent = routePrompt(request);
  return agent.execute(request);
}

function logRoute(agent: AgentDefinition, reason: string): AgentDefinition {
  logAgentDecision({
    etapa: 'roteamento',
    agente: agent.nome,
