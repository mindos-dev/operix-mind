import type { AgentDefinition, AgentRequest, AgentResponse } from './agent-types.js';
...
export function createMockedAgent(config: Omit<AgentDefinition, 'execute' | 'review' | 'summarize'>): AgentDefinition {
  async function buildResponse(request: AgentRequest, prefix: string): Promise<AgentResponse> {
  ...
  return {
    ...
    review: (content) =>
      buildResponse(
        { input: content, taskType: 'revisao', language: 'pt-BR' },
        'Revisão técnica por'
      ),
    summarize: (content) =>
      buildResponse(
        { input: content, taskType: 'resumo', language: 'pt-BR' },
        'Resumo técnico por'
      )
  };
}
