import type { RuntimeTask, TaskComplexity } from './runtime.types.js';

export function classifyTaskComplexity(task: RuntimeTask): TaskComplexity {
  const input = `${task.tipo} ${task.prompt}`.toLowerCase();
  if (
    task.requerAltaPrecisao ||
    task.debugDificil ||
